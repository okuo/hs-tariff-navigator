...
const typeStyles: Record<ToastData['type'], { bg: string; border: string; text: string; icon: string }> = {
  success: {
    bg: 'bg-green-50',
    ...
    <div className="fixed top-2 left-2 right-2 z-50 space-y-2">
      {toasts.map((toast) => {
        ...
        return (
          <div
            key={toast.id}
            className={`${style.bg} ${style.border} border rounded-lg p-3 shadow-md flex items-start animate-fade-in`}
          >
            <svg className={`h-5 w-5 ${style.text} flex-shrink-0 mt-0.5`} fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d={style.icon} clipRule="evenodd" />
            </svg>
            <p className={`ml-2 text-sm ${style.text} flex-1`}>{toast.message}</p>
            <button
              onClick={() => onRemove(toast.id)}
              className={`ml-2 ${style.text} hover:opacity-70 flex-shrink-0`}
              aria-label="閉じる"
            >
